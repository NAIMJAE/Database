...
grant all privileges on `Bank`.* to 'naimjae'@'%';
...
insert into `bank_account` values ('101-23-1007', 'L3', '주택담보대출', '750210-1000002', 2700000 , '2020-09-21');
...
# 실습 2-15
select sum(`t_amount`) as '입금 총합',
avg(`t_amount`) as '입금 평균'
...
# 실습 2-24
select `t_a_no`, `t_dist`, sum(`t_amount`) as `합계` from `bank_transaction` where `t_dist` = 1  
group by `t_a_no` having `합계` >= 100000 order by sum(`t_amount`) desc;
...
on a.`t_a_no` = b.`a_no` order by a.`t_no`;
