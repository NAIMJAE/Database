...
/* 카드종류가 신용카드인 고객에 대한 모든 정보(고객명, 고객주소, 고객생년월일, 고객이메일, 고객전화번호, 고객직업) 조회 */
SELECT A.`cust_name`, A.`cust_addr`, A.`cust_birth`, A.`cust_email`, A.`cust_phnum`, A.`cust_job` FROM `customer` AS A
JOIN `card` AS B ON A.`cust_jumin` = B.`cust_jumin` WHERE `card_type` = 'credit';

/*예금잔고가 4,000,000원 이상 고객에 대한 고객명, 주민번호, 전화번호, 주소 조회*/
SELECT A.`cust_name`, A.`cust_jumin`, A.`cust_phnum`, A.`cust_addr` FROM `customer` AS A
JOIN `account` AS B ON A.`cust_jumin` = B.`cust_jumin` WHERE `acc_balance` >= 4000000;
