...
# Nurses
insert into `nurses` values ('N101101', '송승헌', '1976-02-21', 'M', '101', '수간호사', '010-1101-7602', 'ssb@bw.com');
...
# 문제 5
select a.`treat_no`, c.`pat_name`, b.`doc_name`, `treat_contents`, `chart_contents`, `treat_datetime` from `Treatments` as a
join `doctors` as b on a.`doc_id` = b.`doc_id` 
join `patients` as c on a.`pat_id` = c.`pat_id`
...
join `doctors` as b on a.`doc_id` = b.`doc_id` 
join `patients` as c on a.`pat_id` = c.`pat_id`
join `charts` as d on a.`treat_no` = d.`treat_no` where `treat_contents` like '%화상%';
...
select * from
 ...
 from `patients`) as a where `age` >= 30 and `age` <= 40;
...
left join `doctors` as b on a.`dep_no` = b.`dep_no` where `doc_id` is null;

# 문제 9
select group_concat(`pat_name` separator '^') as `환자` from `nurses` as a
join `patients` as b on a.`nur_id` = b.`nur_id` where `nur_name` = '김태희';

# 문제 10
select a.`nur_id`, `nur_name`, count(*) as `담당환자 수` from `nurses` as a
left join `patients` as b on a.`nur_id` = b.`nur_id`
group by `nur_id` order by `담당환자 수` desc limit 1;

select * from `nurses` as a
left join `patients` as b on a.`nur_id` = b.`nur_id`;
